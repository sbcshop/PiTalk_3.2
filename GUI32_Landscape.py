...
###############################  Icon  #########################################
class Icon:
    """ This class handle icons for PiTalk Button Icons """
    def __init__(self):
        try:
...
class MainApp(pitalk.PiTalk, tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.w = 320  #screen width
        self.h = 240  #screen height
        ...
        self.networkLabel = tk.Label(self.Topframe, text ="",bg = "black",\
                                     fg = "OliveDrab1")
        self.networkLabel.grid(row = 0, column = 0, columnspan = 3,sticky = "w")
        
        self.timeLabelTop = tk.Label(self.Topframe, text = "",bg = "black", \
                                     fg = "tomato")
        self.timeLabelTop.grid(row = 0, column = 3, columnspan = 3,padx=(0,70))
        
        self.msgLabelTop = tk.Label(self.Topframe, bg = "black")
        self.msgLabelTop.grid(row = 0, column = 6)

        self.callLabelTop = tk.Label(self.Topframe,bg = "black")
        self.callLabelTop.place(x=210, y=0)

        self.signal = tk.Canvas(self.Topframe, width=self.bar_width,bg= "black",\
                                height=self.bar_height,highlightthickness=0,bd=0)
        self.signal.grid(row = 0, column = 7, sticky = "e")

        container = tk.Frame(self,height=240, width=320, bg = "black")
        ...
        self.messageData=[]
        self.frames = {}
        for F in (HomeFrame, Menu1Frame, Menu2Frame, CallFrame,CallIncomingFrame,
                  CamFrame, MessageFrame, InboxBodyFrame, InboxFrame, \
                  CreateSMSFrame,DeleteSMSFrame, StopwatchFrame, SettingsFrame,\
                  GUIRotateFrame, CallSettingFrame, CalendarFrame, AudioFrame,\
                  AboutFrame, SIMFrame, GalleryFrame, CalculatorFrame, \
    ...
    def GPIO_init(self):
        """ This function initialize GPIO used of LCD keys """
        GPIO.setup(18, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        GPIO.setup(16, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        GPIO.setup(12, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        GPIO.add_event_detect(12,GPIO.RISING, bouncetime = 200,\
                              callback = self.keyPress)
        GPIO.add_event_detect(16,GPIO.RISING, bouncetime = 200,\
                              callback = self.keyPress)
        GPIO.add_event_detect(18,GPIO.RISING, bouncetime = 200,\
                              callback = self.keyPress)
    
    def keyPress(self, GPIO_PIN):
        """
        This function handle Key Press

        Args:
            GPIO_PIN: GPIO pin number of Key1, Key2 & Key3
        """
        if GPIO_PIN == 12:
            if Gframename == "Menu1Frame":
                self.show_frame("HomeFrame")
            elif Gframename == "Menu2Frame":
                self.show_frame("Menu1Frame")
            elif Gframename == "CamFrame":
                retFrame = self.get_frame("CamFrame")
                retFrame.exitFlag = False
                self.show_frame("Menu1Frame")   
            
        if GPIO_PIN == 16:
            if Gframename == "CamFrame":
                retFrame = self.get_frame("CamFrame")
                retFrame.captureFlag = True
            else:
                self.show_frame("HomeFrame")
              
        if GPIO_PIN == 18:
            if Gframename == "Menu1Frame":
                self.show_frame("Menu2Frame")
            elif Gframename == "CamFrame":
                retFrame = self.get_frame("CamFrame")
                retFrame.exitFlag = False
                self.show_frame("HomeFrame")
    ...
    def update_network(self):
        ...
        if self.Fl_initialize == False and phone.alive == True:
            ...
            self.networkLabel.config(text = self.networkName,fg="OliveDrab1" )
...
class HomeFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.canvas = tk.Canvas(self,bg="black",width = 320, height = 320,\
                                highlightthickness=0,bd=0)
        self.circle = self.canvas.create_rectangle(80,30,230,140,outline="tomato",\
                                                   width=7)     
        ...
        self.dateLabel = tk.Text(self,bg = "black", fg = "yellow" ,height = 5, \
                                 width = 18, bd  = 0, highlightthickness = 0)
        ...
        self.call = tk.Button(self, text = "Call", image=icons[4],bg = "black",\
                              ...
                              activeforeground = "white",command = lambda: \
        ...
        self.camera = tk.Button(self,image=icons[8], text = "Camera",bg = "black",\
                                fg = "white", bd = 0, highlightbackground = "black",\
                                compound="top", activebackground = "black", \
                                activeforeground = "white", command =lambda: \
    ...
    def update_Clock(self):
        """ Update clock after every second """
        ...
        self.dateLabel.tag_config("time" ,foreground = "tomato", justify = "center",\
                                  font=("Helvetica", 15),background = "black")
        self.dateLabel.tag_add("date", "3.0", "end")
        self.dateLabel.tag_config("date" ,justify = "center", font=("Helvetica", 12),\
                                  foreground = "gold")
                
        self.after(1000, self.update_Clock)

    def update_color(self):
        """ Update color after every 10 seconds """
    ...
    def __init__(self, parent, controller):
        ...
        for b in range(len(BUTTON)):
            ...
            tk.Button(self, text = BUTTON[b][0],bg = "white", fg = "black",bd = 0,\
                      ...
                      activeforeground = "black", image = BUTTON[b][1],compound = 'top',\
                      command = cmd).grid(row = BUTTON[b][3], column = BUTTON[b][4],\
...
class CallFrame(tk.Frame):
    """ This class handle dialpad and outgoing call """
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        tk.Frame.config(self, bg="black")
        self.controller = controller
        ...
        self.diallednumber = tk.Entry(self,font=("bold", 15),width=15, \
                                      justify="right",fg='RoyalBlue1')
        self.diallednumber.grid(row = 0, column = 0, columnspan =3)

        for b in range(12):
            cmd = lambda x=BUTTON[b][0]: self.dial_Entry(x)
            self.buttons.append(tk.Button(self,text=BUTTON[b][0],fg="white",\
                                          font=("bold", 15),highlightbackground="black",\
                                          activebackground="black",anchor=BUTTON[b][1],\
                                          activeforeground="white",bg="black",bd = 0,
                                          command = cmd))
            self.buttons[b].grid(row=BUTTON[b][2],column=BUTTON[b][3],sticky="nsew")
        
        self.buttons.append(tk.Button(self, image = icons[5],bd=0, bg = "black", \
                                      fg = "white", highlightbackground = "black", \
                                      activebackground = "black", activeforeground = \
                                      "white",command = self.outgoing_Call))
        self.buttons[-1].grid(row = 5, column = 0, sticky ="nsew", pady=(0,10))

        self.buttons.append(tk.Button(self, image = icons[16],bd=0,bg = "black", \
                                      fg = "white",highlightbackground = "black",\
                                      activebackground = "black", activeforeground =\
                                      "white", command = lambda: controller.\
        ...
        self.buttons.append(tk.Button(self, image = icons[11],bd=0,bg = "black", \
                                      fg = "white",highlightbackground = "black",\
                                      activebackground = "black", activeforeground =\
                                      "white", command = self.dial_Delete))
    ...
    def outgoing_Call(self):
        ...
        if len(self.diallednumber.get()):
            ...
            self.outgoingcall = tk.Label(self, text = "Outgoing Call", fg = "tomato",\
                                         font=("bold", 18), bg = "white")
            self.outgoingcall.grid(row = 0, column = 0, columnspan = 7,sticky = "s")

            self.dialnumber = tk.Label(self, text = self.number, font=("bold", 15),\
                                       fg = "gray30", bg = "white")
            self.dialnumber.grid(row = 1, column = 0, columnspan =7,padx=50,pady=20,\
                                 sticky = "n")
    ...
    def audio_mode(self, args):
        ...
        if args == 1:
            ...
            self.label.after(1000, self.label.grid_forget)
        else:
            self.label.grid(row = 4, column = 1)
            phone.speakerMode()
    ...
    def call_Hangup(self):
        """ Disconnect the outgoing call and go back to HomeFrame """
        phone.hangup()
...
class CallIncomingFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.timer = tk.Label(self, text = "00:00:00", bg = "black", fg = "white",\
                              font = ("Helvetica", 11))
        self.timer.grid(row = 2, column = 0, columnspan =5)

        self.connect = tk.Button(self, image=icons[5],bg = "black", fg = "black",bd = 0,\
                                 highlightbackground = "black", activebackground = \
                                 "black", command = self.call_connect)
    ...
    def call_time_counter(self):
        ...
        if(self.Fl_incoming_call_start):
            self.timer.configure(text = "%02d:%02d:%02d" %(self.hour, self.minute,\
                                                           self.second))
...
class MessageFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        BUTTON = [# Text            Image       Frame          Row Col
                  ...
                  ("Sent Message", icons[25],  None,            2, 0),
        ...
        for b in range(len(BUTTON)):
            ...
            tk.Button(self, text = BUTTON[b][0],bg = "white", fg = "black", bd = 0,\
                      highlightbackground = "white", activebackground = "white",\
                      activeforeground = "black", image = BUTTON[b][1],compound = 'left',\
                      anchor = "w",command = cmd).grid(row = BUTTON[b][3], column =\
                                                       BUTTON[b][4], sticky ="nsew") 

        home = tk.Button(self, bg = "white", fg = "white", bd = 0, image = icons[16],\
                         highlightbackground = "white", activebackground = "white",\
                         activeforeground = "black", command = lambda: controller\
                         .show_frame("HomeFrame"))
        home.grid(row = 4, column = 0, sticky = "new")

#################################################  InboxFrame  #################
class InboxFrame(tk.Frame):
    """ This class handle Inbox messages and make log """
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        tk.Frame.config(self,bg="black")
        self.controller = controller
        ...
        self.vsb = tk.Scrollbar(self, orient = "vertical",width = 15,\
                                command = canvas.yview)
        canvas.configure(yscrollcommand = self.vsb.set)

        self.vsb.pack(side = "right", fill = "y")
        canvas.pack(side = "left", fill = "both", expand = True)
        canvas.create_window((0,0), window = self.frame, width = 320,anchor = "nw")

        self.frame.bind("<Configure>", lambda event, canvas = canvas: \
                        self.onFrameConfigure(canvas))
        self.frame.grid_columnconfigure(0, weight = 1)
        
        self.home = tk.Button(self.frame, image = icons[16], bg = "black",anchor =\
                              "center", bd = 0,highlightbackground = "black", \
    ...
    def update_inbox(self):
        ...
        self.incomingSMS.append(tk.Button(self.frame,bg = "gold2",fg = "black",\
                                          activeforeground = "white",activebackground =\
                                          "black", text = self.controller.messageData\
                                          [self.index][0]+ "\t "+ self.controller.\
                                          messageData[self.index][1],command = lambda \
                                          index=self.index: self.textframe.opne_sms(index)))
...
class InboxBodyFrame(tk.Frame):
    ...
    def opne_sms(self, args):
        ...
        self.bodySMS = tk.Text(self,bg = "white", fg= "black", height = 10, width = 35)
        self.bodySMS.delete(1.0, "end")
        self.bodySMS.insert(1.0, self.controller.messageData[args][2])
        self.bodySMS.grid(row = 2, column = 0, columnspan = 3,padx=40, sticky = "nsew")

        self.reply = tk.Button(self,text = "Reply", bg = "white",bd = 0,highlightbackground =\
                               "white", activeforeground = "black",activebackground = "white",
                               command = lambda args=args: self.reply_msg(args))
        self.reply.grid(row = 3, column = 1)

        self.back = tk.Button(self, bg = "black",bd = 0,highlightbackground = "black",\
                              activeforeground = "black",activebackground = "black",\
                              image = icons[30], command = lambda : self.controller.\
                              show_frame("InboxFrame"))
        self.back.grid(row = 3, column = 0)

        self.backtohome = tk.Button(self, image = icons[16], bg = "black",bd = 0,\
                                    highlightbackground = "black", activeforeground =\
                                    "black",activebackground = "black",command = lambda:\
                                    self.controller.show_frame("HomeFrame"))
...
class CreateSMSFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.Label = tk.Label(self,text = "To:", bg = "gray80")
        self.Label.grid(row = 0, column = 0)

        self.recipient = tk.Entry(self, justify="right", width = 25)
        ...
        self.messageSend = tk.Button(self, text = "SEND", bg='green4',fg='gold', \
                                     command = self.send_Message)
        self.messageSend.grid(row = 0, column = 8, columnspan = 2,sticky = "nsew")

        self.messageBody = tk.Text(self, height = 6, width = 35)
        self.messageBody.grid(row = 1, column = 0, columnspan = 10, sticky = "nsew")
        ...
        for k in range(30):
            ...
            self.buttonCharData[k] = tk.Button(self, text = self.specialChar[k],\
                                               command = lambda k=k:self.messageEntry\
                                               (self.specialChar[k]))
            self.buttonCapsData[k] = tk.Button(self, text = self.letterCaps[k],\
                                               command = lambda k=k:self.messageEntry\
                                               (self.letterCaps[k]))
            self.buttonData[k] = tk.Button(self, text = self.letter[k],\
                                           command = lambda k=k:self.messageEntry\
                                           (self.letter[k]))
        ...
        home = tk.Button(self, text = "HOME",fg = "Deep Sky Blue", command = lambda:\
                         controller.show_frame("HomeFrame"))
        home.grid(row = 5, column = 0, columnspan = 2 , sticky = "nsew")
        
        capital_char = tk.Button(self, text = "UP", command = self.capitalLetters)
        ...
        symbols = tk.Button(self, text = "SYM", command = self.specialCharacters)
        symbols.grid(row = 5, column = 7,sticky = "nsew")

        delete = tk.Button(self, text = "DEL",fg='red4', command = \
                           self.single_Char_Delete)
    ...
    def capitalLetters(self):
        ...
        if(self._buttoncount == 1):
            ...
            for j in range(30):
                ...
                self.butDeleteSMSFrametonData[j].grid()
                self._buttoncount = 0
    ...
    def send_Message(self):
        ...
        if int(self.recipient.get()):
            status = phone.sendSMS(int(self.recipient.get()), self.messageBody.\
                                   get(1.0, "end") )
            if status[-1] == 'OK\r\n':
                self.recipient.delete(0,"end")
...
class DeleteSMSFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.warningLabel = tk.Label(self, bg = "black", fg = "Deep sky blue",\
                                     font=("helvetica", 12),
                                     text = 'Selecting this deletes messages '\
                                            'from \nyour GSM Module only')
        self.warningLabel.grid(row = 0, column = 0, columnspan = 3)

        tk.Label(self, text = "Press OK to confirm",font=("helvetica", 10),\
                 fg = "yellow", bg = "black").grid(row = 1, column = 1)
        
        ok = tk.Button(self, text = "OK" ,fg = "yellow",image = icons[21], \
                       anchor = "center", highlightbackground = "white", bg =\
                       "black", activebackground = "black",activeforeground =\
                       "white", compound = "top", command = self.deletesms)
        ok.grid(row = 2, column = 1)

        back = tk.Button(self, text = "Back",image = icons[30],bd = 0,bg = "black",\
                         highlightbackground = "black", activebackground = "black",\
                         anchor = "center", command = lambda : self.controller.\
                         show_frame("MessageFrame"))
...
class CamFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        tk.Button(self,text="Photo", font=("bold", 12),bg = "black", fg = "white",\
                  bd = 0, highlightbackground = "black",  activebackground = "black",\
                  activeforeground = "white", image = icons[9], compound="top", \
                  command = self.startPreview).place(x = 100, y = 40)

        self.back = tk.Button(self,text = "Home", bg = "black", fg = "black", bd = 0,\
                              image = icons[30], highlightbackground = "black", \
                              activebackground = "black", activeforeground = "black",\
                              command = lambda: controller.show_frame("Menu1Frame"))
        self.back.place(x=20,y=170)

        self.home = tk.Button(self, bg = "black", fg = "black", bd = 0, image = icons[16],\
                              highlightbackground = "black", activebackground = "black",\
                              activeforeground = "black",command = lambda: controller.\
                              show_frame("HomeFrame"))
        self.home.place(x=260,y=170)
        ...
        if(response == b'supported=1 detected=1\n'):
            ...
            while(self.exitFlag):
                if self.captureFlag == True:
                    camera.capture(path + '/Gallery/Photos/' + 'img_' + time.\
                                   strftime('%d%m%Y_') + time.strftime('%H%M%S')\
                                   + '.jpg', use_video_port=False, format='jpeg',\
                                   thumbnail = None)
                    self.captureFlag = False            
            ...
            label = tk.Label(self, text="Error: Camera is not enabled", bg="yellow")
...
class InternetFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        Label = tk.Label(self,text = "APN : ",font=("Bold", 12), bg = "black",\
                         fg = "yellow")
        Label.grid(row = 0, column = 0, columnspan = 2, pady=(10,20))

        self.APN = tk.Entry(self, width = 12,font=("bold", 12))
        self.APN.grid(row = 0, column = 1, columnspan = 6, pady=(10,20))
        self.APN.bind("<FocusIn>", self.remember_Focus)
        self.APN.focus()

        Label = tk.Label(self,text = "PORT : ",font=("Bold", 12),bg = "black",\
                         fg = "deep sky blue")
        Label.grid(row = 1, column = 0, columnspan = 2, pady=(0,50))

        self.Port = tk.Entry(self, width = 12,font=("Bold", 12))
        self.Port.grid(row = 1, column = 1, columnspan = 6, pady=(0,50))
        self.Port.bind("<FocusIn>", self.remember_Focus)
        self.Port.focus()

        self.connect = tk.Button(self, text = "Connect", compound = "top",\
                                 fg = "green", command = self.connect)
        self.connect.place(x=220, y=8)

        self.disconnect = tk.Button(self, text = "Disconnect", compound = "top",\
                                    fg = "red", command = self.disconnect)
        self.disconnect.place(x=210, y=50)

        self.status = tk.Button(self, text = "Status",command = self.check_status)
        self.status.place(x=225, y=85)
        ...
        for k in range(30):
            ...
            self.buttonCharData[k] = tk.Button(self,font = 1, text = self.specialChar[k],\
                                               command = lambda k=k:self.messageEntry\
                                               (self.specialChar[k]))
            self.buttonCapsData[k] = tk.Button(self, font = 1, text = self.letterCaps[k],\
                                               command = lambda k=k:self.messageEntry\
                                               (self.letterCaps[k]))
            self.buttonData[k] = tk.Button(self, font = 1, text = self.letter[k],\
                                           command = lambda k=k:self.messageEntry\
                                           (self.letter[k]))
        ...
        home = tk.Button(self, text = "HOME", fg = "Deep Sky Blue", command = \
                         lambda: controller.show_frame("HomeFrame"))
    ...
    def connect(self):
        ...
        if (len(self.APN.get()) and len(self.Port.get())):
            subprocess.call(shlex.split('sudo ' + path+'/PPP/ppp_create'+ \
                                        str(self.APN.get())+" "+str(self.Port.get())))
            subprocess.Popen('sudo ' + path + '/PPP/ppp_start', shell=True)
            self.status.config(state = "normal")
            self.connect.config(state = "disable")

        else:
            label = tk.Label(self, text = "Enter values", bg = "yellow", fg = "black")
    ...
    def check_status(self):
        ...
        if "ppp0" in str(subprocess.check_output(["ifconfig"])):
            label = tk.Label(self, text = "Connected", bg = "OliveDrab", fg = "white")
            label.place(x=20, y=85)
            label.after(2000, label.place_forget)

        else:
            label = tk.Label(self, text = "Not Connected", bg = "red", fg = "black")
            label.place(x=20, y=85)
            label.after(2000, label.place_forget)
...
class GalleryFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.photo = tk.Button(self,text="Photos", bg = "white", fg = "black", \
                               bd = 0, highlightbackground = "white",image = icons[9],\
                               activebackground = "white", activeforeground = "black",\
                               compound="top", command = self.view_images)
        self.photo.place(x=110, y=40)

        self.home = tk.Button(self, bg = "white", fg = "white", bd = 0, image = icons[16],\
                              highlightbackground = "white", activebackground = "white",\
                              activeforeground = "white", command = self.photo_home)
        self.home.place(x=140,y=180)

        self.prev = tk.Button(self, text = "<<", bg = "white", fg = "black", bd = 0, \
                              highlightbackground = "white",  activebackground = "white",\
                              activeforeground = "black", state = "disable", command = \
                              self.prev_image)
        self.prev.place(x=60, y=190)

        
        self.next = tk.Button(self, text = ">>", bg = "white", fg = "black", bd = 0,\
                              highlightbackground = "white",activebackground = "white",\
                              activeforeground = "black", state="disable", command = \
                              self.next_image)
        self.next.place(x=210, y=190)

        self.label = tk.Label(self,text = "",bg="white", fg="black")
        self.label.place(x=255, y=195)
        
    def view_images(self):
        """ Open Images """
        self.homestate = "Photo"
        self.photo.config(state = "disable")
        for file in sorted(os.listdir(path + '/Gallery/Photos')):
        ...
        if len(self.images):
            ...
            self.label.config(text= str(self.index + 1) + "/"+ str(self.totalImages))
...
class CalculatorFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.calculatorentry = tk.Entry(self, width = 30, font=("helvetica", 12))
        self.calculatorentry.grid(row = 0, column = 0, columnspan = 4, sticky = "nsew")

        self.square = tk.Button(self, text = "x²",font=("helvetica", 12), width = 3,\
                                bg = "black", fg = "white", command = self.square)
        self.square.grid(row = 1, column = 0, sticky = "nsew")

        self.sqroot = tk.Button(self, text = "√",font=("helvetica", 12), width = 3,\
                                bg = "black", fg = "white", command = self.squareroot)
        self.sqroot.grid(row = 1, column = 1, sticky = "nsew")

        self.AC = tk.Button(self, text = "AC", width = 3,font=("helvetica", 12),\
                            bg = "black", fg = "white", command = self.deleteall)
        self.AC.grid(row = 1, column = 2, sticky = "nsew")

        self.C = tk.Button(self, text = "C", width = 3, font=("helvetica", 12), \
                           bg = "black", fg = "white", command = self.delete1)
        ...
        for k in range(16):
            ...
            self.calculatorKeyData[k] = tk.Button(self, text = self.calculatorKeys[k],\
                                                  width = 3, bg = "black", fg = "white",\
                                                  font=("helvetica", 12), command = lambda\
                                                  k=k:self.entryaction(self.calculatorKeys[k]))
        ...
        tk.Button(self, text = "=", font=("helvetica", 14), bg = "black",fg = "white",\
                  command = self.equals).grid(row = 6, column = 2, columnspan = 2, \
                                              sticky = "nsew")

        tk.Button(self, bg = "black", fg = "red", text = "Back",font=("helvetica", 12),
                  command = self.back).grid(row = 6, column = 0,  sticky = "nsew")
        
        tk.Button(self, bg = "black", fg = "Deep sky blue",text = "Home",font=("helvetica", 12),
                  command = self.home).grid(row = 6, column = 1, sticky = "nsew")
...
class StopwatchFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.canvas = tk.Canvas(self,bg="black",width = 320, height = 200,
                                     highlightthickness=0,bd=0)
        self.canvas.create_rectangle(80,30,230,120,outline="royal blue",width=7)
        self.canvas.grid(row = 0, column = 0, rowspan = 3,columnspan = 3, sticky =\
                         "nsew")

        tk.Label(self, text = "HH : MM : SS", font=("helvetica", 10),fg = "white",\
                 bg = "black").place(x = 115, y = 40)
        
        self.stopwatch = tk.Label(self,bg = "black", fg = "yellow", text = "00:00:00",\
                                  font=("helvetica", 20), bd  = 0,highlightthickness = 0)
        self.stopwatch.grid(row = 1, column = 1)

        self.startwatch = tk.Button(self, text = "Start", image=icons[29],bg = "black",\
                                    fg = "white", bd=0, highlightbackground = "black",\
                                    activebackground = "black", activeforeground = "black",\
                                    command = self.start_watch)
        self.startwatch.grid(row = 2, column = 0, sticky="s")

        self.stopWatch = tk.Button(self, text = "Stop",image=icons[37], bg = "black", \
                                   fg = "white", bd=0, highlightbackground = "black",\
                                   activebackground = "black", activeforeground = "black",\
                                   command = self.stop_watch)
        self.stopWatch.grid(row = 2, column = 1,sticky = "s")

        self.reset = tk.Button(self, text = "Reset",image=icons[31], bg = "black",\
                               fg = "white", bd=0, highlightbackground = "black",\
                               activebackground = "black", activeforeground = "black",\
                               command = self.reset_watch)
        self.reset.grid(row = 2, column = 2, sticky = "s")

        goback = tk.Button(self, bg = "black", fg = "black", bd = 0, image = icons[30],\
                           highlightbackground = "black",  activebackground = "black",\
                           activeforeground = "black", text = "Home", command = self.back)
        goback.grid(row = 3, column = 0)
        gohome = tk.Button(self, bg = "black", fg = "black", bd = 0, image = icons[16],\
                           highlightbackground = "black",  activebackground = "black",\
                           activeforeground = "black", text = "Home", command = self.home)
        gohome.grid(row = 3, column = 2)
...
class CalendarFrame(tk.Frame):
    ...
    def setup(self, y, m):
        """ Setup """
        left_month = tk.Button(self, text='<',font=("helvetica", 12), bg = "black",\
                               fg = "tomato", command=self.go_prev_month)
        self.wid.append(left_month)
        left_month.grid(row=0, column=0)
        right_month = tk.Button(self, text='>', font=("helvetica", 12),bg = "black",\
                                fg = "tomato",command=self.go_next_month)
        self.wid.append(right_month)
        right_month.grid(row=0, column=1)
                 
        header = tk.Label(self, bg = "black", fg = "SlateBlue1", font=("helvetica", 12),\
                          text='{}   {}'.format(calendar.month_abbr[m], str(y)))
        self.wid.append(header)
        header.grid(row=0, column=2, columnspan=3)
        
        left_year = tk.Button(self, text='∨', bd = 0,font=("helvetica", 11),\
                              bg = "black", fg = "tomato",command=self.go_prev_year)
        self.wid.append(left_year)
        left_year.grid(row=0, column=5)         
        right_year = tk.Button(self, text=' ∧', bd = 0,font=("helvetica", 11), \
                               bg = "black", fg = "tomato",command=self.go_next_year)
        self.wid.append(right_year)
        right_year.grid(row=0, column=6)
                 
        
        for num, name in enumerate(self.DAYS):
            t = tk.Label(self, text=name[:3], bg = "black", fg = "gold",font=("helvetica", 10))
            self.wid.append(t)
            t.grid(row=1, column=num, pady=6)
                 
        for w, week in enumerate(self.cal.monthdayscalendar(y, m), 2):
            for d, day in enumerate(week):
                if day:
                    b = tk.Button(self, width=1, text=day, bg = "black", fg = "white",\
                                  font=("helvetica", 10))
                    self.wid.append(b)
                    b.grid(row=w, column=d)

        ok = tk.Button(self, image = icons[30], bg = "black", fg = "black", bd = 0,\
                       highlightbackground = "black", activebackground = "black",\
                       activeforeground = "black", command = lambda: self.controller.\
                       show_frame("Menu2Frame"))
        self.wid.append(ok)
        ok.grid(row=8, column=0, columnspan=3, pady=10)
        
        home = tk.Button(self, image = icons[16], bg = "black", bd=0, activeforeground =\
                         "black",highlightbackground = "black", activebackground = "black",
...
class AudioFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.headphone = tk.Button(self, text = "Headphone \nMode", image  = icons[15],\
                                   bg = "black", fg = "white",bd = 0,highlightbackground =\
                                   "black", activebackground = "black", activeforeground =\
                                   "white",compound = "top", anchor = "w", command = \
                                   self.headphone_mode)
        self.headphone.grid(row = 0, column = 0, ipadx = 0, sticky = "s")

        self.speaker = tk.Button(self, text = "Speaker \nMode",image  = icons[34],\
                                 bg = "black", fg = "white",bd = 0,compound = "top",\
                                 highlightbackground = "black",activebackground = \
                                 "black", activeforeground = "white", anchor = "w",\
                                 command = self.speaker_mode)
        self.speaker.grid(row = 0, column = 2,sticky = "s")

        self.slider = tk.Scale(self, from_=0, to=7, orient = "horizontal", bg = \
                               "black", fg = "white", bd = 0, highlightbackground = \
                               "black",state = "disable")
        self.slider.grid(row = 2, column = 0, columnspan = 3, sticky = "nsew", padx = 50)

        self.gainset = tk.Button(self,text = "Set", state = "disable", fg = "white",\
                                 bg = "black", command = self.set_Volume)
        self.gainset.grid(row = 3, column = 1)

        backbutton = tk.Button(self, text = "Back", image = icons[30], bg = "black",\
                               fg = "black", bd = 0, highlightbackground = "black",
                               activebackground = "black", activeforeground = "black",\
                               command = lambda: controller.show_frame("Menu2Frame"))
        backbutton.grid(row = 4, column = 0,sticky = "nsew")

        homebutton = tk.Button(self, text = "Home", image = icons[16], bg = "black",\
                               fg = "black", bd = 0, highlightbackground = "black",
                               activebackground = "black", activeforeground = "black",\
                               command = lambda: controller.show_frame("HomeFrame"))
    ...
    def headphone_mode(self):
        """ Headphone Mode """
        self.slider.config(label = "Headphone gain", state = "disable", fg = "yellow")
    ...
    def speaker_mode(self):
        """ Speaker Mode """
        self.slider.config(label = "Speaker gain", state = "normal", fg = "deep sky blue")
...
class SensorFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        tk.Label(self,text = "Read ADC Values" ,font=("helvetica", 15),bg = "black",\
                 fg="red").grid(row = 0, column = 0, columnspan = 5)
        tk.Label(self,text = "Measure Voltage Range 0v - 2.1v" , bg = \
                 "yellow").grid(row = 1, column = 0, columnspan = 5, sticky="n")


        tk.Label(self,text = "ADC 0" , bg = "black",font=("helvetica", 13),\
                 fg="gold2").grid(row = 2, column = 0, columnspan=2,sticky="n")
        tk.Button(self, text="Read ADC (mV)", command = self.read_ADC0).\
                        grid(row = 3, column = 0, sticky='n')
        
        self.adc0 = tk.Entry(self,width = 7,font=("helvetica", 12))
        self.adc0.grid(row = 4, column = 0,sticky='n', pady=(0,10))
        
        tk.Label(self,text = "ADC 1" ,font=("helvetica", 12), bg = "black",\
                 fg="gold2").grid(row = 2, column = 3, columnspan=2,sticky='n')
        tk.Button(self, text="Read ADC (mV)",command = self.read_ADC1).\
                        grid(row = 3, column = 3, sticky='n')
        
        self.adc1 = tk.Entry(self,  width = 7,font=("helvetica", 12))
        self.adc1.grid(row = 4, column = 3,sticky='n', pady=(0,10))

        tk.Button(self, bg = "black", fg = "black", bd = 0, image = icons[30],\
                  highlightbackground = "black",  activebackground = "black", \
                  activeforeground = "black",command = self.back).\
                  grid(row = 5, column = 0)
        tk.Button(self, bg = "black", fg = "black", bd = 0, image = icons[16],\
                  highlightbackground = "black",  activebackground = "black", \
                  activeforeground = "black", command = self.home).\
                  grid(row = 5, column = 3)
...
class PiFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.warningLabel = tk.Label(self, bg = "black", fg = "Deep sky blue",\
                                     font=("helvetica", 14), text = "This feature"\
                                     " allows you to access\n your Raspberry Pi by"\
                                     " shutting\n down the phone UI")
        self.warningLabel.grid(row = 0, column = 0, columnspan = 3, sticky = "nsew")
        
        tk.Label(self, text = "Press OK to confirm",  font=("helvetica", 12),fg = \
                 "yellow", bg = "black").grid(row = 1, column = 1)
        
        ok = tk.Button(self, text = "OK" ,fg = "yellow",image = icons[18], bg="black",\
                       anchor = "center", highlightbackground = "white", compound = "top",\
                       activebackground = "black",activeforeground = "white",\
                       command = self.closeUI)
        ok.grid(row = 2, column = 1)

        back = tk.Button(self, text = "Back",image = icons[30],bd = 0,highlightbackground =\
                         "black", bg = "black", activebackground = "black", anchor = "center",\
                         command = lambda : self.controller.show_frame("Menu2Frame"))
        back.grid(row = 3, column = 0)

        tk.Button(self, text = "Go Home", image = icons[16], anchor = "center",bd = 0,\
                  highlightbackground = "black", bg = "black", activebackground = "black",\
                  command = lambda: controller.show_frame("HomeFrame")).grid(row = 3, column = 2)
...
class AboutFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        LABEL = [
                  ("PiTalk is a 3G GSM Shield for  RPi", "olive drab"),
                  ("  ->Quectel UC15 Chipset", "white"),
                  ("  ->Compatible with 3.2", "white"),
                  ("  ->Quad band UTMS/HSPDA Module", "white"),
                  ("  ->Compatiable with Pi2/Pi3/PiZero", "white"),
                  ("For More Info:", "turquoise"),
                  ("    www.sb-components.co.uk", "white"),
                  ("Developed by:", "tomato"),
                  ("  Ankur Singh ", "white"),
                  ("  Asher Ahmad Farooqui", "white")]
        ...
        header_label = tk.Label(self, text = "PiTalk", font=("helvetica", 14),\
                                fg = "gold2", bg = "black")
        header_label.grid(row = 0, column = 3, sticky = "ew")

        for i in range(len(LABEL)):
            tk.Label(self, text = LABEL[i][0], bg = "black", fg = LABEL[i][1]).\
                           grid(row = i+1, column = 0, columnspan = 5, sticky = "w")

        back_button = tk.Button(self, image = icons[30], bg = "black", fg = "black",\
                                bd = 0, highlightbackground = "black", activebackground =\
                                "black", activeforeground = "black", command = lambda: \
                                controller.show_frame("Menu2Frame"))
        back_button.grid(row = 12, column = 1, sticky = "e")

        homebutton = tk.Button(self, image = icons[16], bg = "black", fg = "black", bd = 0,\
                               highlightbackground = "black", activebackground = "black",\
                               activeforeground = "black", command = lambda: controller.\
                               show_frame("HomeFrame"))
        homebutton.grid(row = 12, column = 4)
...
class SIMFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        tk.Label(self, text = "IMEI Number:",font=("helvetica", 15), bg = "black",\
                 fg = "orange red").pack()

        self.imeiLabel = tk.Label(self, text = "",font=("helvetica", 12),bg = \
                                  "black", fg = "yellow")
        self.imeiLabel.pack()

        tk.Label(self, text = "Module: ",font=("helvetica", 15),bg = "black",\
                 fg = "orange red").pack()

        tk.Label(self, text = "Quectel UC15",font=("helvetica", 12),bg = "black",\
                 fg = "yellow").pack()

        tk.Label(self, text = "Carrier:",font=("helvetica", 15),bg = "black",\
                 fg = "orange red").pack()

        self.networkLabel = tk.Label(self, text = "",font=("helvetica", 12),\
                                     bg = "black", fg = "yellow")
        self.networkLabel.pack()

        tk.Button(self, text="Read",bg = "black", fg = "Deep sky blue", command = \
                  self.read).pack()

        tk.Button(self, image = icons[30], bg = "black", fg = "black", bd = 0,\
                  highlightbackground = "black", activebackground = "black",\
                  activeforeground = "black", command = lambda: controller.\
                  show_frame("Menu2Frame")).pack(side = "left")

        tk.Button(self, image = icons[16], bg = "black", fg = "black", bd = 0,\
                  highlightbackground = "black", activebackground = "black",\
                  activeforeground = "black", command = lambda: controller.\
                  show_frame("HomeFrame")).pack(side = "right")
...
class SettingsFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        BUTTON = [# Text             Image       Frame            row col
                  ("Orientation",   icons[28], "GUIRotateFrame",   0, 0,),
                  ("Call Settings", icons[7],  "CallSettingFrame", 0, 1,),
                  ("HDMI Mode",     icons[43], "HDMIFrame",        1, 0,),
                  ("Logs",          icons[46], "LogFrame",         1, 1,),
                   ]
        ...
        for b in range(4):
            cmd = lambda x=BUTTON[b][2]: controller.show_frame(x)
            tk.Button(self, text = BUTTON[b][0], image = BUTTON[b][1],bg = "black",\
                      fg = "white", anchor = "w", bd = 0, highlightbackground = \
                      "black",activeforeground = "white", activebackground = "black",\
                      compound = 'left', command = cmd).\
                      grid(row = BUTTON[b][3], column = BUTTON[b][4], sticky = "nsew")
        
        tk.Button(self, text = "Shutdown", image = icons[44], bg = "black", fg = \
                  "white", anchor = "w", bd = 0, highlightbackground = "black",\
                  activeforeground = "white", activebackground = "black", compound = \
                  'left', command = lambda: controller.show_frame("ShutdownFrame")).\
                  grid(row = 4, column = 0, columnspan=2)
        
        tk.Button(self, text = "Go Home", image = icons[16], fg = "white", bd = 0,\
                  anchor = "center",command = lambda: controller.show_frame("HomeFrame")).\
...
class GUIRotateFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        tk.Label(self, text ="This will reboot your system and \ncalibrate the screen",\
                 font=("bold", 12),bd = 2,fg = "tomato", bg = "black").\
                 grid(row = 0, column = 0,columnspan = 2 , sticky = "nsew")
        
        rotate0 = tk.Button(self, text = "Rotate 0°  ", bg = "black", fg = "white",\
                            command = lambda:self.screenRotate(0), bd = 2,\
                            highlightbackground = "black",compound="left",\
                            activebackground = "black", activeforeground = "white",\
                            image  = icons[39])
        rotate0.grid(row = 1, column = 0)

        rotate90 = tk.Button(self, text = "Rotate 90° ", bg = "black", fg = "white",\
                             command = lambda:self.screenRotate(90), bd = 2,\
                             highlightbackground = "black",compound="left", \
                             activebackground = "black", activeforeground = "white",\
                             image  = icons[40])
        rotate90.grid(row = 1, column = 1)

        rotate180 = tk.Button(self, text = "Rotate 180°", bg = "black", fg = "white",\
                              command = lambda:self.screenRotate(180), bd = 2,\
                              highlightbackground = "black",compound="left",\
                              activebackground = "black", activeforeground = "white",\
                              image  = icons[41])
        rotate180.grid(row = 2, column = 0)

        rotate270 = tk.Button(self, text = "Rotate 270°",  bg = "black", fg = "white",\
                              command = lambda:self.screenRotate(270), bd = 2,\
                              highlightbackground = "black",compound="left",\
                              activebackground = "black", activeforeground = "white",\
                              image  = icons[42])
        ...
        goback = tk.Button(self, text = "go Back",image = icons[30], bg = "black",\
                           fg = "black", bd = 0, highlightbackground = "black",\
                           activebackground = "black", activeforeground = "black",\
    ...
    def screenRotate(self, arg):
        """ Screen Rotate """
        if arg == 0:
            subprocess.call(['./LCD32-show','0'], cwd = path+'/LCD-show')
        elif arg == 90:
            subprocess.call(['./LCD32-show','90'], cwd = path+'/LCD-show')
        if arg == 180:
            subprocess.call(['./LCD32-show','180'], cwd = path+'/LCD-show')
        if arg == 270:
            subprocess.call(['./LCD32-show','270'], cwd = path+'/LCD-show')
...
class CallSettingFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        canvas = tk.Canvas(self, bg = "snow", height = 10).\
                 grid(row = 0, column = 0, columnspan = 6 , sticky = "nsew")
        
        tk.Label(self,bg = "snow", fg = "black", text = "Auto Answer rings:").\
                         grid(row = 0, column = 0, columnspan = 3)

        self.ringspin = tk.Spinbox(self,from_=0, to = 10, width = 6)
        self.ringspin.grid(row = 0, column = 2, columnspan = 3)
        
        tk.Button(self,text = "Set",bg = "deep sky blue", fg = "white", command =\
                  self.ring_Set).grid(row = 0, column = 5)

        tk.Label(self,bg = "black", fg = "white", text = "Forward Call :").\
                         grid(row = 1, column = 0, columnspan = 2)

        self.forwardnumber = tk.Entry(self, justify="right",fg='RoyalBlue1',\
                                      width=15, font=("helvetica", 12))
        self.forwardnumber.grid(row = 1, column = 2, columnspan = 4, sticky='w')
        
        tk.Button(self,text = "Enable", command = self.set_callforwarding).\
                            grid(row = 2, column = 4, columnspan = 2)
       
        for b in range(12):
            cmd = lambda x=BUTTON[b][0]: self.dial_Entry(x)
            self.buttons.append(tk.Button(self,text=BUTTON[b][0],bg = "black",\
                                          font=("helvetica",12),highlightbackground="black",\
                                          activebackground = "black", activeforeground="white",\
                                          fg="white",bd=0,command = cmd))
            self.buttons[b].grid(row=BUTTON[b][2],column=BUTTON[b][3])

    
        tk.Button(self,text = "Check",bg = "black",fg = "white",command = self.check_status).\
                            grid(row = 2, column = 0,columnspan = 2)

        tk.Button(self,text = "Disable",bg = "black",fg = "white",command = self.disable_callforwarding).\
                            grid(row = 2, column = 2, columnspan = 2)
        
        tk.Button(self,image = icons[16],bg = "black",fg = "white",bd = 0,highlightbackground = "black",\
                  activebackground = "black", activeforeground = "black", command = self.home).\
                  grid(row = 6, column = 5)

        tk.Button(self,image = icons[30],bg = "black",fg = "black",bd = 0,highlightbackground = "black",\
                  activebackground = "black", activeforeground = "black", command = lambda: controller.\
                  show_frame("SettingsFrame")).grid(row = 6, column = 0)

        tk.Button(self, image = icons[11],bd=0,bg = "black", fg = "white",highlightbackground = "black",\
                  activebackground = "black", activeforeground = "white", command = self.dial_Delete).\
                  grid(row = 6, column = 4)
    ...
    def ring_Set(self):
        ...
        if int(self.ringspin.get()):
            ...
            phone.autoRing(int(self.ringspin.get()))
            label.config(text = "Auto Ring \nNot Enabled", bg = "yellow", fg = "black")
            label.grid(row = 3, column = 0, rowspan = 2, sticky = "w")
            label.after(2000, label.grid_forget)
    ...
    def set_callforwarding(self):
        """ Set call forwading """
        label = tk.Label(self)
        if int(self.forwardnumber.get()):
            if phone.enableCallForwarding(int(self.forwardnumber.get())):
                label.config(text = "Enabled", bg = "yellow", fg = "black")
                label.grid(row = 3, column = 0)
                label.after(2000, label.grid_forget)
            else:
                label.config(text = "Number Format \nNot Correct", bg = "yellow", \
                         fg = "black")
                label.grid(row = 3, column = 0, rowspan = 2, sticky = "w")
                label.after(2000, label.grid_forget)
...
class HDMIFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, bg = "black", fg = "Deep sky blue",font=("bold", 12),\
                         text = "Selecting this reboots and changes \noutput to HDMI")
        label.grid(row = 0, column = 0, columnspan = 3, sticky = "nsew")
        
        okLabel = tk.Label(self, text = "Press OK to confirm",font=("bold", 11),\
                           fg = "yellow", bg = "black")
        okLabel.grid(row = 1, column = 1)
        
        okButton = tk.Button(self, text = "OK" ,fg = "yellow",image = icons[43],\
                             anchor = "center",highlightbackground = "white",\
                             bg = "black", activebackground = "black",compound = "top",\
                             activeforeground = "white",  command = self.hdmi_mode)
        ...
        home = tk.Button(self, text = "Go Home", image = icons[16], anchor = "center",\
                         bd = 0,highlightbackground = "black", bg = "black",\
                         activebackground = "black", command = lambda: controller.\
                         show_frame("HomeFrame"))
...
class LogFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.enable = tk.Button(self, text = "Enable Log",image  = icons[47],\
                                command = self.enable_log, bg = "black", bd=0,\
                                fg = "white", highlightbackground = "black",\
                                compound="left",state="disable", activebackground =\
                                "black", activeforeground = "white")
        self.enable.grid(row = 1, column = 0)

        self.disable = tk.Button(self, text = "Disable Log",image  = icons[48],\
        ...
        delete = tk.Button(self, text = " Delete All Logs ", image  = icons[49],\
                           command = self.delete_log, bg = "black", fg = "white",\
                           bd = 0, highlightbackground = "black",compound="left",\
                           activebackground = "black", activeforeground = "white")
        delete.grid(row = 2, column = 0, columnspan = 3)
        
        back = tk.Button(self, text = "Back",image = icons[30],bd = 0, bg = "black",\
                         highlightbackground = "black",  anchor = "center",\
                         activebackground = "black", command = lambda : self.\
    ...
    def enable_log(self):
        """ Enable logging """
        self.Fl_logs = True
        self.status.config(text="Logs Enabled", bg="green")
    ...
    def delete_log(self):
        """ Delete all the logs """
        label = tk.Label(self,bg="black", fg="black" )
...
class ShutdownFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, bg = "black", fg = "Deep sky blue",font=("Helvetica",13),\
                         text = "Selecting this shuts down your \nGSM Module and UI")
        label.grid(row = 0, column = 0, columnspan = 3)
        
        okLabel = tk.Label(self, text = "Press OK to confirm",font=("Helvetica",11),\
                           fg = "yellow", bg = "black")
        okLabel.grid(row = 1, column = 1, sticky = "nsew")
        
        okButton = tk.Button(self, text = "OK" ,fg = "yellow",image = icons[44], \
                             anchor = "center", activebackground = "black",bg = "black",\
                             highlightbackground = "white",  activeforeground = "white",\
                             compound = "top", command = self.shutdown)
        okButton.grid(row = 2, column = 1)

        back = tk.Button(self, text = "Back",image = icons[30],bd = 0,  bg = "black",\
                         highlightbackground = "black", anchor = "center", \
                         activebackground = "black", command = lambda : self.controller.\
                         show_frame("SettingsFrame"))
        back.grid(row = 3, column = 0)
...
class LocationFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, bg = "black", fg="Deep sky blue",font=("helvetica", 14),\
                         text = "Get location coordinates")
        label.grid(row = 0, column = 0, columnspan = 2)
        
        self.lattitude = tk.Label(self, text="Lattitude: " ,bg = "black", fg = "tomato",\
                                  font=("helvetica", 12), bd = 2, highlightbackground = \
                                  "black", activebackground = "black", activeforeground =\
                                  "white")
        self.lattitude.grid(row = 1, column = 0, sticky = "nsew", columnspan = 2)

        self.longitude = tk.Label(self, text = "Longitude: ",bg = "black", fg = "tomato",\
                                  font=("helvetica", 12), bd = 2, highlightbackground = \
                                  "black", activebackground = "black", activeforeground =\
                                  "white")
        self.longitude.grid(row = 2, column = 0, sticky = "nsew", columnspan = 2)

        getposition = tk.Button(self, text = " Get Position ",image  = icons[14],bd = 2,\
                                command = self.get_position,bg="black", compound="left",\
                                highlightbackground = "black", activebackground = "black",\
                                activeforeground = "white",fg = "white")
        getposition.grid(row = 3, column = 0,columnspan = 2)
        ...
        home = tk.Button(self, text = "Go Home", image = icons[16], anchor = "center",bd = 0,\
                         bg = "black", highlightbackground = "black", activebackground = \
                         "black", command = self.gohome)
        home.grid(row = 4, column = 1)

    def get_position(self):
        """ Get coordinates """
        self.lattitude.config(text = "Lattitude: "+str(phone.get_location()[0])+"°")
        self.longitude.config(text = "Longitude: "+str(phone.get_location()[1])+"°")
...
class TemplateFrame(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text = "Develop\nyour apps here", bg = "black",\
                         fg = "Deep sky blue",font=("helvetica", 15),)
        label.grid(row = 0, column = 0, columnspan = 2)
        ...
        back = tk.Button(self, text = "Back",image = icons[30],bd = 0,bg = "black",\
                         ...
                         anchor = "center", command = lambda: self.controller.\
                         show_frame("Menu2Frame"))
        back.grid(row = 1, column = 0, sticky="s")
        
        home = tk.Button(self, text = "Go Home", image = icons[16],bd = 0,\
                         anchor = "center",highlightbackground = "black",\
                         bg = "black", activebackground = "black", command = \
                         lambda: self.controller.show_frame("Menu2Frame"))
        home.grid(row = 1, column = 1,sticky="s")
